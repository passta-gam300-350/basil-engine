...
RegisterResourceTypeForward(MeshMetaRuntimeData, "meshmeta", meshmetadef)
 ...
 * @brief Component for rendering text in world-space (3D), similar to Unity's TextMesh.
 * Uses Transform component for positioning/rotation. Supports billboard modes.
 */
struct TextMeshComponent {
    rp::BasicIndexedGuid m_FontGuid{ static_cast<rp::BasicIndexedGuid>(rp::TypeNameGuid<"font_atlas">{}) };  ///< GUID of the font atlas asset

    // Text content
    std::string text = "World Text";  ///< Text string (UTF-8 encoded)

    /**
     * @brief Billboard mode for world-space text
     */
    ...
    enum class Alignment : uint8_t {
        ...
        Right,
        Justified
    } alignment = Alignment::Center;  ///< Default center for world text

    // Layout properties
    float lineSpacing = 1.0f;     ///< Line spacing multiplier (1.0 = default line height)
    float letterSpacing = 0.0f;   ///< Letter spacing in world units (scaled by distance)
    float maxWidth = 0.0f;        ///< Maximum width for word wrapping in world units (0 = no wrapping)
    ...
    // Glow/shadow effect
    float glowStrength = 0.0f;                              ///< Glow/shadow strength (scaled by distance)
    glm::vec4 glowColor = glm::vec4(0.0f, 0.0f, 0.0f, 0.5f); ///< Glow/shadow color (RGBA)
