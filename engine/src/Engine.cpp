...
// Editor facade implementations - encapsulate Graphics library access
void Engine::ProcessObjectPicking(int screenX, int screenY, int viewportWidth, int viewportHeight, PickingResultData& outResult) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (!sceneRenderer) {
	...
	query.screenX = screenX;
	query.screenY = screenY;
	query.viewportWidth = viewportWidth;
	query.viewportHeight = viewportHeight;
	...
	// Log picking result for debugging
	spdlog::info("Engine::ProcessObjectPicking: Query at ({}, {}) in viewport ({}x{}) -> hasHit: {}, objectID: {}, worldPos: ({:.2f}, {:.2f}, {:.2f})",
		screenX, screenY, viewportWidth, viewportHeight,
		result.hasHit, result.objectID,
		result.worldPosition.x, result.worldPosition.y, result.worldPosition.z);

	// Store results
	outResult.hasHit = result.hasHit;
	outResult.objectID = result.objectID;
	outResult.worldPosition = result.worldPosition;
	outResult.depth = result.depth;
}

void Engine::SetAmbientLightColor(float r, float g, float b) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (sceneRenderer) {
		sceneRenderer->SetAmbientLight(glm::vec3(r, g, b));
	}
}

void Engine::SetAABBVisualization(bool show) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (sceneRenderer) {
		sceneRenderer->SetAABBVisualization(show);
	}
}

void Engine::GetEditorFrameTexture(FrameTextureData& outData) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (!sceneRenderer) {
	...
	if (frameData.editorResolvedBuffer && frameData.editorResolvedBuffer->GetFBOHandle() != 0) {
		outData.textureID = frameData.editorResolvedBuffer->GetColorAttachmentRendererID(0);
		outData.isValid = true;
		outData.deltaTime = frameData.deltaTime;
	} else {
		outData.isValid = false;
	}
}

void Engine::SetSelectedEntity(uint32_t entityID) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (sceneRenderer) {
		sceneRenderer->ClearOutlinedObjects();
		sceneRenderer->AddOutlinedObject(entityID);
		spdlog::info("Engine: Set selected entity for outline: {}", entityID);
	}
}

void Engine::ClearSelectedEntity() {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (sceneRenderer) {
		sceneRenderer->ClearOutlinedObjects();
		spdlog::info("Engine: Cleared selected entity outline");
	}
}

void Engine::SetEditorViewportSize(int width, int height) {
	auto* sceneRenderer = GetRenderSystem().m_SceneRenderer.get();
	if (sceneRenderer) {
		sceneRenderer->SetEditorViewportSize(width, height);
	}
}
