/******************************************************************************/
/*!
\file   GameResolvePass.cpp
\author Team PASSTA
\par    Course : CSD3401 / UXG3400
\date   2026/01/16
\brief  Game resolve pass implementation
...
*/
/******************************************************************************/
#include "../../include/Pipeline/GameResolvePass.h"
#include "../../include/Pipeline/RenderContext.h"
#include <spdlog/spdlog.h>
#include <glad/glad.h>

GameResolvePass::GameResolvePass()
    : RenderPass("GameResolvePass", FBOSpecs{ 1, 1, {{FBOTextureFormat::RGBA8}} })  // Minimal valid spec (unused)
{
    // This pass doesn't render to its own framebuffer
    // It creates a separate gameResolvedBuffer for the game viewport
...
void GameResolvePass::Execute(RenderContext &context)
{
    // Determine which buffer to use for game viewport display (same logic as EditorResolvePass)
    ...
    // Create final game buffer - keep it simple with same format as source
    // We'll rely on pure glBlitFramebuffer (proven to work without tearing)
    if (!context.frameData.gameResolvedBuffer ||
        context.frameData.gameResolvedBuffer->GetSpecification().Width != sourceSpec.Width ||
        context.frameData.gameResolvedBuffer->GetSpecification().Height != sourceSpec.Height)
    {
        ...
        context.frameData.gameResolvedBuffer = std::make_shared<FrameBuffer>(resolvedSpec);

        spdlog::debug("GameResolvePass: Created game buffer ({}x{}, same format as source)",
    ...
    {
        sourceBuffer->ResolveToFramebuffer(context.frameData.gameResolvedBuffer.get());
    }
    else
    {
        glBindFramebuffer(GL_READ_FRAMEBUFFER, sourceBuffer->GetFBOHandle());
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, context.frameData.gameResolvedBuffer->GetFBOHandle());
...
void GameResolvePass::CreateFullScreenQuad()
{
    ...
    spdlog::info("GameResolvePass: Full-screen quad created");
}
